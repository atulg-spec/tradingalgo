...
from stratergy.management.commands.dhan.dhanhq import marketfeed
...
async def on_message(instance, message):
    ...
    try:
        bid_qty = 0
        ask_qty = 0
        for x in message['depth']:
            bid_qty = int(x['bid_quantity']) * int(x['bid_orders'])
            ask_qty = int(x['ask_quantity']) * int(x['ask_orders'])
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Start the WebSocket feed
            feed.run_forever()
            print('RUN AGAIN')
            os.system('python3 manage.py dhan_websocket3')
